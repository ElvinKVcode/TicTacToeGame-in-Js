...
function checkWin() {
	...
	for (let row = 0; row < 3; row++) {
		if (gameBoard[row * 3] !== '' && gameBoard[row * 3] === gameBoard[row * 3 + 1] && gameBoard[row * 3] === gameBoard[row * 3 + 2]) {
			return true
		}
	}
	// Burda sütunları yoxla
	for (let col = 0; col < 3; col++) {
		if (gameBoard[col] !== '' && gameBoard[col] === gameBoard[col + 3] && gameBoard[col] === gameBoard[col + 6]) {
			return true
		}
	}
